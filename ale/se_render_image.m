...
%=======================================================================
	...
	num   = 1;% spm_input('Number of sets',1,'1 set|2 sets|3 sets',[1 2 3]);
...
% get surface
%-----------------------------------------------------------------------
    rendfile = fullfile(spm('dir'),'rend','render_single_subj.mat'); % spm_select(1,'^render.*\.mat$','Render file');
...
if numel(dat) == 1 && min(dat.t) == max(dat.t)
    brt = 0.01;
end
...
if nargin < 5
    if ~isfinite(brt) 
        PLTdepth = 25;
    elseif min(dat(1).t) == max(dat(1).t)
        PLTdepth = 30;
    else
        PLTdepth = 35;
    end
